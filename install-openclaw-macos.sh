...
if [[ "${OSTYPE:-}" != darwin* ]]; then
  echo "Script nay chi danh cho macOS."
  exit 1
fi
...
OPENCLAW_APP_SHA256="${OPENCLAW_APP_SHA256:-}"
APP_DMG_PATH="${APP_DMG_PATH:-$HOME/Downloads/OpenClaw-latest.dmg}"
OPENCLAW_HOST_HOME="${OPENCLAW_HOST_HOME:-$HOME/.openclaw}"
OPENCLAW_APP_CONFIG_PATH="${OPENCLAW_APP_CONFIG_PATH:-$OPENCLAW_HOST_HOME/openclaw.json}"
...
mkdir -p "$INSTALL_DIR/config" "$INSTALL_DIR/data/postgres" "$INSTALL_DIR/data/openclaw/config" "$INSTALL_DIR/data/openclaw/workspace" "$INSTALL_DIR/scripts" "$OPENCLAW_HOST_HOME"
...
VIRTUAL_KEY="$(curl -fsS "http://127.0.0.1:${LITELLM_PORT}/key/generate" \
  ...
  -H "Content-Type: application/json" \
  --data '{"models":["claude-opus-4-6","gpt-5.1-codex","MiniMax-M2.5","MiniMax-M2.1"],"metadata":{"app":"openclaw","owner":"macos-auto"},"key_alias":"openclaw","duration":"30d"}' \
  | python3 - <<'PY'
import json, sys
...
if [[ -z "$OPENCLAW_APP_URL" ]]; then
  readarray -t APP_META < <(python3 - <<'PY'
import json, urllib.request
release = json.load(urllib.request.urlopen("https://api.github.com/repos/openclaw/openclaw/releases/latest"))
asset = next(a for a in release["assets"] if a["name"].endswith(".dmg"))
print(asset["browser_download_url"])
print(asset.get("digest", ""))
PY
)
  OPENCLAW_APP_URL="${APP_META[0]}"
  if [[ -z "$OPENCLAW_APP_SHA256" && "${#APP_META[@]}" -ge 2 ]]; then
    OPENCLAW_APP_SHA256="${APP_META[1]#sha256:}"
  fi
fi

echo "Downloading OpenClaw.app ..."
curl -L --fail --progress-bar -o "$APP_DMG_PATH" "$OPENCLAW_APP_URL"

if [[ -n "$OPENCLAW_APP_SHA256" ]]; then
  ACTUAL_SHA="$(shasum -a 256 "$APP_DMG_PATH" | awk '{print $1}')"
  if [[ "$ACTUAL_SHA" != "$OPENCLAW_APP_SHA256" ]]; then
    echo "Checksum mismatch for $APP_DMG_PATH"
    exit 1
  fi
fi

MOUNT_LINE="$(hdiutil attach "$APP_DMG_PATH" -nobrowse -noautoopen | tail -n 1)"
MOUNT_POINT="${MOUNT_LINE##*$'\t'}"
ditto "$MOUNT_POINT/OpenClaw.app" "/Applications/OpenClaw.app"
hdiutil detach "$MOUNT_POINT" >/dev/null

cat >"$OPENCLAW_APP_CONFIG_PATH" <<EOF
{
  "gateway": {
    "mode": "remote",
    "remote": {
...
echo "Waiting for macOS app pairing request..."
PENDING_FILE="$INSTALL_DIR/data/openclaw/config/devices/pending.json"
for _ in $(seq 1 90); do
  if python3 - "$PENDING_FILE" <<'PY'
...
try:
    ...
    print("0")
    raise SystemExit(0)
print("1" if isinstance(data, dict) and data else "0")
PY
  then
    if [[ "$(python3 - "$PENDING_FILE" <<'PY'
import json, pathlib, sys
...
docker exec oc-openclaw-gateway node dist/index.js devices approve --latest || true

echo
echo "OpenClaw macOS stack is ready."
echo "Install dir: $INSTALL_DIR"
echo "Dashboard: http://127.0.0.1:${OPENCLAW_GATEWAY_PORT}/#token=${OPENCLAW_GATEWAY_TOKEN}"
echo "macOS app: /Applications/OpenClaw.app"
echo
echo "Neu ban dung macOS companion app, hay cap quyen cho OpenClaw trong:"
echo "- Privacy & Security > Accessibility"
echo "- Privacy & Security > Screen Recording"
echo "- Privacy & Security > Automation"
echo "- Sau nay doi provider key, sua $INSTALL_DIR/.env roi chay $INSTALL_DIR/scripts/apply-provider-config.sh"
echo "- Smoke test 1 lenh: $INSTALL_DIR/scripts/test-model.sh"
echo
if [[ -n "$TELEGRAM_BOT_TOKEN" ]]; then
  echo "Telegram bot da duoc bat. Pairing user Telegram van se la buoc rieng khi co nguoi DM bot."
fi
